...
* **Segregation**: Create focused interfaces following Interface Segregation Principle (ISP)
* **Abstraction**: Define contracts at the appropriate abstraction level for each layer
* **Naming Convention**: Use descriptive names with 'I' prefix (e.g., `ITaskRepository`, `ITaskService`)
* **Dependency Direction**: Higher layers depend on interfaces defined in lower layers
...
{
    Task<TaskDto> CreateTaskAsync(CreateTaskCommand command, CancellationToken cancellationToken = default);
    ...
    Task<TaskDto> UpdateTaskAsync(Guid id, UpdateTaskCommand command, CancellationToken cancellationToken = default);
    Task DeleteTaskAsync(Guid id, CancellationToken cancellationToken = default);
}
```

**Infrastructure Interfaces** (Application Layer):
```csharp
...
* **Constructor Injection**: Always inject interfaces, never concrete implementations
* **Async by Default**: Use `async Task` for all operations that may involve I/O
* **Cancellation Support**: Include `CancellationToken` parameters for long-running operations
* **Return Abstractions**: Return interfaces or DTOs, never expose internal implementations
* **Exception Contracts**: Document expected exceptions in interface documentation
...
* **Application Services**: Orchestrate domain operations and coordinate with infrastructure through interfaces.
...
* **Dependency Injection**: Use constructor injection to acquire interface dependencies.
* **Interface Contracts**: Define infrastructure service contracts (IEmailService, ICacheService, etc.).

### Infrastructure Layer

* **Repositories**: Implement repository interfaces defined in the domain layer for aggregate persistence and retrieval.
* **Event Bus**: Implement event publishing interfaces for domain events.
* **Data Mappers / ORMs**: Map domain objects to database schemas via repository implementations.
* **External Service Adapters**: Implement infrastructure service interfaces for external system integration.
* **Interface Implementation**: All infrastructure services must implement contracts defined in higher layers.
