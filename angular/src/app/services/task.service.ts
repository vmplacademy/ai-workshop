...
export class TaskService {
  ...
  deleteTask(taskId: string | number) { // Support both string and number IDs
    this.tasks.update(tasks => 
      tasks.filter(task => task.id != taskId) // Use != for string/number comparison
    );
  }
  ...
  setSorting(sortBy: 'dueDate' | 'status' | 'taskName', order: 'asc' | 'desc' = 'asc') {
    this.sortBy.set(sortBy);
    this.sortOrder.set(order);
  }

  // Helper method to get task by ID
  getTaskById(id: string | number): Task | undefined { // Support both string and number IDs
  ...
  toggleTaskStatus(taskId: string | number) { // Support both string and number IDs
